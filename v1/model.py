...
def generate_complex_model():
    ...
    history_x_location_outputs = keras.layers.Dense(16, activation="relu")(history_x_location_x)

    history_y_location_inputs = keras.Input(shape=(HISTORY_LENGTH*2, ), name="history_y_position")
    history_y_location_x = keras.layers.Dense(16, activation="relu")(history_y_location_inputs)
    ...
    img_x = keras.layers.Dropout(0.2)(img_x)
    ...
    x = keras.layers.concatenate([history_x_location_outputs, history_y_location_outputs, history_action_outputs, img_outputs])
    x = keras.layers.Dense(16, activation="relu")(x)
    y = keras.layers.Dense(number_of_actions, activation="softmax")(x)

    model = keras.Model(
        inputs=[history_x_location_inputs, history_y_location_inputs, history_action_inputs, img_inputs],
        outputs=y
    )
