...
def generate_complex_model():
    ...
    history_x_location_x = keras.layers.Dropout(0.2)(history_x_location_x)
    history_x_location_outputs = keras.layers.Dense(16, activation="relu", name="history_x_position_output")(history_x_location_x)

    history_y_location_inputs = keras.Input(shape=(HISTORY_LENGTH*2, ), name="history_y_position")
    history_y_location_x = keras.layers.Dense(4, activation="relu")(history_y_location_inputs)
    history_y_location_x = keras.layers.Dropout(0.2)(history_y_location_x)
    history_y_location_x = keras.layers.Dense(8, activation="relu")(history_y_location_x)
    history_y_location_x = keras.layers.Dropout(0.2)(history_y_location_x)
    history_y_location_outputs = keras.layers.Dense(4, activation="relu", name="history_y_position_output")(history_y_location_x)

    history_action_inputs = keras.Input(shape=(HISTORY_LENGTH*2, ), name="history_action")
    history_action_x = keras.layers.Dense(8, activation="relu")(history_action_inputs)
    ...
    history_action_x = keras.layers.Dropout(0.2)(history_action_x)
    history_action_outputs = keras.layers.Dense(8, activation="relu", name="history_action_output")(history_action_inputs)
    ...
    history_img_x = keras.layers.Conv3D(8, (2, 2, 2), activation='relu')(history_img_x)
    history_img_x = keras.layers.MaxPool3D((1, 2, 2))(history_img_x)
    history_img_x = keras.layers.Dropout(0.5)(history_img_x)
    history_img_x = keras.layers.Flatten()(history_img_x)
    history_img_outputs = keras.layers.Dense(16, activation='relu', name="history_image_output")(history_img_x)

    img_inputs = keras.Input(shape=(img_rows, img_cols, 3), name="image")
    img_x = keras.layers.Conv2D(64, (3, 3), padding='same', activation='relu')(img_inputs)
    img_x = keras.layers.MaxPool2D((2, 2))(img_x)
    img_x = keras.layers.Dropout(0.5)(img_x)
    img_x = keras.layers.Conv2D(128, (3, 3), padding='same', activation='relu')(img_x)
    img_x = keras.layers.MaxPool2D((2, 2))(img_x)
    img_x = keras.layers.Dropout(0.5)(img_x)
    img_x = keras.layers.Conv2D(128, (3, 3), padding='same', activation='relu')(img_x)
    img_x = keras.layers.MaxPool2D((2, 2))(img_x)
    ...
    img_outputs = keras.layers.Dense(64, activation='relu', name="image_output")(img_x)
